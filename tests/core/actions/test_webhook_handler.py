...
def test_webhook_execute_posts_payload():
    ...
    assert "X-Signature" not in responses.calls[0].request.headers
...
def test_webhook_execute_network_error_does_not_leak_secret():
    with patch(
        ...
        side_effect=requests.ConnectionError("boom https://example.com/hooks/monitor SECRET_XYZ"),
    ):
        handler = WebhookActionHandler()
        result = handler.execute(
            ProposedAction(
...
def test_webhook_execute_fails_without_url():
    handler = WebhookActionHandler()
    result = handler.execute(
        ProposedAction(
            type="webhook", risk_score=0.2,
...
def test_webhook_execute_resolves_encrypted_secret():
    ...
    result = handler.execute(
        ProposedAction(
            type="webhook", risk_score=0.2,
            payload={"url": "https://example.com/hooks/monitor", "secret": encrypted, "site_id": 1},
            description="POST to webhook",
        )
    )
    assert result.success is True
    ...
    if isinstance(raw_body, bytes):
        raw_body = raw_body.decode("utf-8")
    expected = hmac.new(plaintext.encode(), raw_body.encode(), hashlib.sha256).hexdigest()
    assert request.headers["X-Signature"] == f"sha256={expected}"
